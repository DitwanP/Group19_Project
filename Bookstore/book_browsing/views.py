...
def topSeller(request):
    bookResult = books.objects.all()
    return render(request, 'top_sellers.html', {'books': bookResult, "count": bookResult.count()})
...
def filterData(request):
    ...
    if sortType == 'nameAsc':
        results = books.objects.filter(Q(genre=pageName)).order_by('name')
    elif sortType == 'nameDesc':
        results = books.objects.filter(Q(genre=pageName)).order_by('name').reverse()
    elif sortType == 'priceHighToLow':
    ...
    elif sortType == 'releasedDateAsc':
        results = books.objects.filter(Q(genre=pageName)).order_by('releasedDate').reverse()
    elif sortType == 'releasedDateDesc':
        results = books.objects.filter(Q(genre=pageName)).order_by('releasedDate')
    elif sortType == 'authorAsc':
        results = books.objects.filter(Q(genre=pageName)).order_by('author')
    elif sortType == 'authorDesc':
        results = books.objects.filter(Q(genre=pageName)).order_by('author').reverse()
    else:
...
def filterReleasedData(request):
    sortType = request.GET.get("sort_type")
    if sortType == 'nameAsc':
        results = books.objects.all().order_by('name')
    elif sortType == 'nameDesc':
        results = books.objects.all().order_by('name').reverse()
    elif sortType == 'releasedDateAsc':
        results = books.objects.all().order_by('releasedDate').reverse()
    elif sortType == 'releasedDateDesc':
        results = books.objects.all().order_by('releasedDate')
    elif sortType == 'priceHighToLow':
        results = books.objects.all().order_by('price', 'releasedDate').reverse()
    elif sortType == 'priceLowToHigh':
        results = books.objects.all().order_by('price', 'releasedDate')
    elif sortType == 'authorAsc':
        results = books.objects.all().order_by('author', 'releasedDate')
    elif sortType == 'authorDesc':
        results = books.objects.all().order_by('author', 'releasedDate').reverse()
    else:
